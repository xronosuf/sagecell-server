...
old_code_limit = """        if len(code) > 65000:
            self.set_status(413)
...
if old_code_limit not in h:
    raise SystemExit(
        "Could not find upstream SageCell 65000-character /service limit."
    )

if h.count(old_code_limit) != 1:
    raise SystemExit(
        "Expected exactly one upstream SageCell code-size limit."
    )

h = h.replace(old_code_limit, new_code_limit, 1)
